...
if __name__=='__main__':
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        if (epoch+1)%5 == 0:
            torch.save(netG_A2B.state_dict(), 'output/netG_cycle_A2B_withdet_%d.pth'%(epoch+1))
            torch.save(netG_B2A.state_dict(), 'output/netG_cycle_B2A_withdet_%d.pth'%(epoch+1))
            torch.save(netD_A.state_dict(), 'output/netD_cycle_A_withdet_%d.pth'%(epoch+1))
            torch.save(netD_B.state_dict(), 'output/netD__cycle_B_withdet_%d.pth'%(epoch+1))
    ###################################
