...
if __name__=='__main__':
    ...
    netDet.load_state_dict(torch.load('./output/ObjectDet_Swin_T_mesh', map_location=torch.device('cpu'))['model'])
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        if (epoch+1)%5 == 0:
            torch.save(netG_A2B.state_dict(), 'output/netG_retina_A2B_withdet_%d.pth'%(epoch+1))
            torch.save(netG_B2A.state_dict(), 'output/netG_retina_B2A_withdet_%d.pth'%(epoch+1))
            torch.save(netD_A.state_dict(), 'output/netD_retina_A_withdet_%d.pth'%(epoch+1))
            torch.save(netD_B.state_dict(), 'output/netD_retina_B_withdet_%d.pth'%(epoch+1))
    ###################################
