...
if os.path.exists('./checkpoints/sac_buffer_ObjectDet_Train_mesh_1'):
    train_memory = ReplayMemory(20000, 42)
    train_memory.load_buffer('./checkpoints/sac_buffer_ObjectDet_Train_mesh_20000')
# else:
...
if os.path.exists('./checkpoints/sac_buffer_ObjectDet_Test_mesh'):
    test_memory = ReplayMemory(2000, 42)
    test_memory.load_buffer('./checkpoints/sac_buffer_ObjectDet_Test_mesh')
# else:
#     test_memory = ReplayMemory(2000, 42)
#     env = World(SEVER_ID, URDF_PATH, URDF_OBJECT, MAX_STEP, p.GUI, False)
#     for i in tqdm(range(test_memory.capacity)):
#         state = env.reset()
#         for i in range(20):
#             state = env.step([0]*7)
#         obs, _ = p.getBasePositionAndOrientation(env.object)
#         test_memory.push(state, obs)
#     test_memory.save_buffer('ObjectDet', 'Test_pure')
#     env.close()

agent = Train(model=DetModel(4,'swin_tiny_patch4_window7_224'))
writer = SummaryWriter('runs/Object_detect_pure_swin-ti_20k')
...
for i_episode in range(1500):
    ...
    test_loss = agent.test(train_memory, batch_size)
    writer.add_scalar('Loss/Train', train_loss,i_episode)
    writer.add_scalar('Loss/Test', test_loss, i_episode)
    error_test[i_episode%50] = test_loss
    print('{}|{}  Train error:{:.4f}  Test error:{:.4f}'.format(i_episode,1500,train_loss,test_loss))
agent.save_checkpoint('ObjectDet_Swin_Ti_20k')
